...
def test_build_e1kv_computation_applies_foreign_tax_ceiling_and_keeps_kz899_separate() -> None:
    result = build_e1kv_computation(
        ...
        non_reporting_funds=NonReportingFundsInputs(
            deemed_distributed_income=Decimal("1089.5114"),
            domestic_dividends_kz189=Decimal("0.7655"),
            domestic_dividend_kest_kz899=Decimal("0.1914"),
        ),
    )
    ...
    assert result.domestic_dividends_kz189_total == Decimal("2.0000")
    assert result.domestic_dividend_kest_kz899_total == Decimal("0.5000")
    assert result.creditable_foreign_tax_source_sum == Decimal("127.9479")
    assert result.post_loss_base == Decimal("971.845412")
    assert result.foreign_tax_ceiling == Decimal("267.257488")
...
def test_render_output_markdown_contains_new_kz189_kz899_lines() -> None:
    core = CoreInputs(
    ...
    non_reporting = NonReportingFundsInputs(
        fund_distributions=Decimal("0"),
        deemed_distributed_income=Decimal("7"),
        domestic_dividends_kz189=Decimal("0.1"),
        domestic_dividend_kest_kz899=Decimal("0.04"),
        creditable_foreign_tax=Decimal("0"),
    )
    result = build_e1kv_computation(core=core, reporting_funds=reporting, non_reporting_funds=non_reporting)

    rendered = render_output_markdown(
        tax_year=2025,
        core=core,
        reporting_funds=reporting,
        non_reporting_funds=non_reporting,
        result=result,
    )
    ...
    assert "min(1 + 0.3 + 0, 8.69)" in rendered
