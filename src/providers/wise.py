...
def process_wise_statement(csv_file_path: str, exchange_rates_df: pl.DataFrame) -> pl.DataFrame:
    print("\n\n======================== Processing Wise Statement ========================\n")

    # Convert the extracted data into Polars DataFrames
    statement_df = read_csv_to_df(csv_file_path)
    statement_df = statement_df.filter(pl.col("TransferWise ID").str.starts_with("BALANCE_CASHBACK")).select(
    ...
    gross_amount_recovered_df = converted_euro_df.with_columns(
        [
            (pl.col(Column.amount) * 100 / 70).alias(Column.profit),
            (pl.col(Column.amount_euro) * 100 / 70).alias(Column.profit_euro),
        ]
    )

    gross_amount_recovered_df = gross_amount_recovered_df.with_columns(
        [
            (pl.col(Column.profit) - pl.col(Column.amount)).alias(Column.withholding_tax),
            (pl.col(Column.profit_euro) - pl.col(Column.amount_euro)).alias(Column.withholding_tax_euro),
        ]
    )
    logging.debug(gross_amount_recovered_df)

    tax_df = calculate_kest(
        gross_amount_recovered_df, amount_col=Column.profit_euro, tax_withheld_col=Column.withholding_tax_euro
    )
    ...
    summary_df = tax_df.select(
        pl.sum(Column.amount_euro).alias(Column.amount_euro_received_total),
